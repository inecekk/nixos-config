...
*/
  ...
  programs.mpv = {
    ...
    # 1. 核心与渲染参数
    config = {
      # --- 壁纸/全局优化 (硬解 + 默认静音 + 无缝循环) ---
      audio = "no";                # 默认禁用所有音频（专为 mpvpaper 动态壁纸优化）
      hwdec = "auto-safe";          # 优先安全硬件解码 (AMD 6800H 核显 VA-API)
      loop-file = "inf";           # 单文件循环播放
      loop-playlist = "inf";       # 播放列表循环播放
