...
in
{
        ...
        layer-rule {
                match namespace="^noctalia-(bar-[^\"]+|notification|dock|panel|attached-panel|osd)$"  // 匹配 Noctalia 各图层
                background-effect {
                        xray false                                  // 关闭透视效果
                        blur false                                  // 关闭背景模糊（核心设置）
                }
        }

        spawn-at-startup "qs" "-c" "noctalia-shell"                // 开机启动 Noctalia 状态栏/桌面壳

        spawn-at-startup "sh" "-c" "sleep 3 && sudo systemctl start dae.service"  // 延迟 3 秒后启动 dae 代理服务
